...
* **One Turnstile token for all links:** the host accepts a single Turnstile token for every file — no per-file binding, no single-use check server-side (verified empirically: one 730-char token resolved multiple files repeatedly, still valid after 3+ minutes). The first resolve stashes its token and every other link is resolved with a plain `POST /f/<id>/go` — no browser needed. This applies to **both** the direct-link panel/copy actions and the **download workers**: a 20-part archive needs one ~5s solve plus ~0.3s per part instead of one solve per part. The token is refreshed on expiry (conservative 240s TTL) and on rejection the task falls back to a normal browser solve, so site throttling degrades gracefully instead of failing.
...
* **Stale profile locks:** an orphaned Chromium (crash, kill -9) leaves `SingletonLock`/`SingletonSocket` in the profile dir; the next launch silently hands off to the dead instance and dies with *"Failed to connect to browser"*. These are cleared before each browser start.
